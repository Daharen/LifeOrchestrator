...
MemoryResult FileMemoryStore::append_behavioral_reevaluation_artifact(const BehavioralReevaluationArtifact& record) { try { auto it = behavioral_reevaluations_by_id_.find(record.behavioral_reevaluation_id); if (it != behavioral_reevaluations_by_id_.end() && it->second.reevaluated_at == record.reevaluated_at && it->second.backlog_count == record.backlog_count && it->second.intervention_count == record.intervention_count) return ok_result(); behavioral_reevaluations_by_id_[record.behavioral_reevaluation_id] = record; append_line(behavioral_file(data_root_, "reevaluations"), serialize_fields({{"behavioral_reevaluation_id", record.behavioral_reevaluation_id}, {"reevaluated_at", record.reevaluated_at}, {"source_module_id", record.source_module_id}, {"backlog_count", std::to_string(record.backlog_count)}, {"intervention_count", std::to_string(record.intervention_count)}, {"reevaluated_backlog_item_ids", serialize_list(record.reevaluated_backlog_item_ids)}, {"intervention_ids", serialize_list(record.intervention_ids)}, {"version", std::to_string(record.version)}})); return ok_result(); } catch (const std::exception& e) { return error_result(e.what()); } }
