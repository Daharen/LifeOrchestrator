...
MemoryResultWith<BehavioralMemorySummary> FileMemoryStore::get_behavioral_memory_summary() const { return success(BehavioralMemorySummary{behavioral_proposals_by_id_.size(), behavioral_state_snapshots_by_id_.size(), behavioral_decisions_by_id_.size(), behavioral_backlog_items_by_id_.size(), behavioral_interventions_by_id_.size()}); }
...
MemoryResult FileMemoryStore::load_from_disk() {
    ...
    try {
        ...
        for (const auto& line : read_lines(behavioral_file(data_root_, "backlog"))) { auto f = parse_fields(line); behavioral_backlog_items_by_id_[f["backlog_item_id"]] = {f["backlog_item_id"], f["behavioral_proposal_id"], backlog_status_from_string(f["status"]), f["deferred_reason"], f["first_deferred_at"], f["last_reconsidered_at"].empty()?std::nullopt:std::optional<TimestampString>(f["last_reconsidered_at"]), f["reconsider_after"].empty()?std::nullopt:std::optional<TimestampString>(f["reconsider_after"]), f["source_module_id"], static_cast<std::uint64_t>(std::stoull(f["version"]))}; behavioral_backlog_item_id_by_proposal_id_[f["behavioral_proposal_id"]] = f["backlog_item_id"]; }
        for (const auto& line : read_lines(behavioral_file(data_root_, "interventions"))) { auto f = parse_fields(line); behavioral_interventions_by_id_[f["intervention_id"]] = {f["intervention_id"], f["behavioral_proposal_id"], f["behavioral_decision_id"], f["title"], intervention_presentation_mode_from_string(f["presentation_mode"]), f["scheduled_for"].empty()?std::nullopt:std::optional<TimestampString>(f["scheduled_for"]), f["created_at"], f["status"], f["source_module_id"], static_cast<std::uint64_t>(std::stoull(f["version"]))}; }
        ...
        for (const auto& line : read_lines(procedural_file(data_root_, "optimization_proposals"))) { auto f = parse_fields(line); optimization_proposals_by_id_[f["optimization_proposal_id"]] = {f["optimization_proposal_id"], f["procedural_audit_run_id"], f["activity_inventory_item_id"], optimization_opportunity_type_from_string(f["opportunity_type"]), effort_value_classification_from_string(f["effort_value_classification"]), {std::stoi(f["recovered_minutes_per_week"]), std::stoi(f["recovered_effort_points"]), f["confidence_label"]}, f["title"], f["rationale"], f["source_module_id"], f["created_at"], f["updated_at"], static_cast<std::uint64_t>(std::stoull(f["version"])), f["linked_behavioral_proposal_id"], f["triage_status"], f["triage_decision_id"], parse_map(f["attributes"])}; }
        snapshot_version_ = entities_by_id_.size() + relationships_by_id_.size() + commitments_by_id_.size() + behavioral_proposals_by_id_.size() + behavioral_state_snapshots_by_id_.size() + activity_inventory_by_id_.size() + optimization_proposals_by_id_.size();
